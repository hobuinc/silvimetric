...
def s3_storage_config(
    s3_uri, bounds, resolution, crs, attrs, metrics
) -> Generator[StorageConfig, None, None]:
    yield StorageConfig(
        bounds, crs, resolution, attrs, metrics, svversion, tdb_dir=s3_uri
    )
...
def s3_shatter_config(
    s3_storage, copc_filepath, attrs, metrics, date
...
def uneven_storage_config(
    tmp_path_factory, bounds, crs, attrs, metrics
) -> Generator[StorageConfig, None, None]:
    log = Log('INFO')
...
def uneven_shatter_config(
    copc_filepath, uneven_storage_config, date
) -> Generator[ShatterConfig, None, None]:
...
def partial_storage_config(
    tmp_path_factory, crs, attrs, metrics
) -> Generator[StorageConfig, None, None]:
    path = tmp_path_factory.mktemp('test_tdb')
    p = os.path.abspath(path)
    log = Log('DEBUG')
...
def partial_shatter_config(
    copc_filepath, date, partial_storage_config
) -> Generator[ShatterConfig, None, None]:
